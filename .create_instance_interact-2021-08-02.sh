...
getServerID(){
    serverip=$(sed -n '2p' /opt/stone/vncc/vncc.conf)
    serverssh=$(sed -n '9p' /opt/stone/vncc/vncc.conf)
    gpu_server="${serverip#*= }":"${serverssh#*= }"
}

NV_GPU=${nvgpu} nvidia-docker create -it --shm-size 64G -v ${pdir}:${vdir}  --name=${name} ${imagename} /bin/bash
...
docker start $name
docker exec --user root $name bash -c "mkdir -p /etc/vncc/">/dev/null
...
docker exec --user root $name bash -c "conda init bash" >/dev/null
